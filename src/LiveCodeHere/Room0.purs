...
import WAGS.Lib.Latch (ALatchAP, MakeLatchAP, LatchAP)
...
class R2SRL i' o | i' -> o where
  r2sRL :: Proxy i' -> { | o }

instance r2sCons :: (IsSymbol a, Lacks a x, R2SRL c x, Cons a String x o) => R2SRL (RL.Cons a b c) o where
  r2sRL _ = Record.insert (Proxy :: _ a) (reflectSymbol (Proxy :: _ a)) (r2sRL (Proxy :: _ c))

instance r2sNil :: R2SRL RL.Nil () where
  r2sRL _ = {}

class R2S i o | i -> o where
  r2s :: { | i } -> { | o }

instance r2sAll :: (RowToList i i', R2SRL i' o) => R2S i o where
  r2s _ = r2sRL (Proxy :: _ i')
...
notesf = hmap toLN notes
  ...
  toLN =
    (\(a :| b) -> (a :| List.fromFoldable b))
      >>> map (\(Note a b c) -> Note (toNumber a / 127.0) (toNumber b) (toNumber c))

names :: Instruments String
names = r2s notes
...
pwfs = hmap toAPFofT notesf
  ...
  toPWF (Note a b c) =
    wrap
      { gain: makePiecewise ((0.0 /\ a) :| (lenny /\ a) : ((lenny + 4.0) /\ 0.0) : Nil)
      , onOff: makeTerracedR ((0.0 /\ On) :| ((lenny + 4.0) /\ Off) : Nil)
      , "_eq": b
      }
    where
    lenny = c - b
...
tmul = tempo / 60.0 :: Number

type Acc r
  = ( room0Instruments :: Instruments (ALatchAP GOO) | r )

actualizePWF :: TimeHeadroom -> (TimeHeadroom -> AudioParameter_ GOO) -> AudioParameter_ GOO
actualizePWF = (#)

actualizer :: Extern -> { room0Instruments :: Instruments (AudioParameter_ GOO) }
actualizer (SceneI { time: time', headroom: headroom' }) = { room0Instruments: hmap (actualizePWF { time, headroom }) pwfs }
  where
  time = time' * ticksInBeat * tempo
  headroom = toNumber headroom' / 1000.0

graph :: forall r. Extern -> { room0Instruments :: Instruments (LatchAP GOO) | r } -> _
graph _ _ = {}
